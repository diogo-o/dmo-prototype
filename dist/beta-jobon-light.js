(()=>{
 ...
 function renderList(){
   ...
   const matches=records.filter(r=>r.date===selectedDate()&&`${r.reference} ${r.production} ${r.line}`.toLowerCase().includes(q)).sort((a,b)=>a.line.localeCompare(b.line)||a.reference.localeCompare(b.reference));
